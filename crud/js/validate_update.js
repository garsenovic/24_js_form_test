function validate() {
    ...
    with(document) {
     ...
     /*   if(getElementById("description").value.length > 4) {
        ...
        if(getElementById("price").value  < 0) {
            getElementById("priceinfo").innerHTML = "Der Wert muss positiv sein";
            getElementById("price").style.backgroundColor = "red";
            getElementById("price").value = "";
            getElementById("price").focus();
            fehler = true;
        }
        if(getElementById("price").value == "") {
            getElementById("priceinfo").innerHTML = "Das Feld darf nicht leer sein";
            getElementById("price").style.backgroundColor = "red";
            getElementById("price").value = "";
            getElementById("price").focus();
            fehler = true;
   ...
   /* if(!fehler) {
        fehler = false;
        f.action = "update.php?id=<?php echo $id?>";
        f.submit();
